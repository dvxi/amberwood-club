...
export default function ProductComponent (props) {
  ...
  const moveY = -203 + props.mousePos.y + props.data.y;
  ...
  const imageStyle = {
    width: '100px',
    height: '207px',
    transform: 'translate(' + moveX + 'px, ' + moveY + 'px) scale(' + (1.0 - Math.abs((((props.data.x + props.mousePos.x) / (window.innerWidth / 2)) * .85)))*scale + ')',
  ...
  return (
    <div className="d-flex align-items-center justify-content-center py-5 position-absolute top-50 start-50" style={imageStyle} onMouseEnter={() => setScale(1.2)} onMouseLeave={() => setScale(1.0)}>
      <Image fluid src="/images/ProductComponentImage.png" style={{cursor: 'pointer'}}/>
    </div>
  );
